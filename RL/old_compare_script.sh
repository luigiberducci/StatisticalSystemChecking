...
do
    ...
    then
        ...
        python3 ${script} --search MC --episodes 50000 --step 100 --outdir ${output_dir} > ${output_dir}/${i}_log_MC.txt
    fi
    # Run ISplit if flag > 0
    if [[ $isplit_flag -gt 0 ]]
    then
        echo "[Info] Iter. ${i} | Importance Splitting..."
        python3 ${script} --search ISplit --episodes 50000 --step 10 --outdir ${output_dir} > ${output_dir}/${i}_log_ISplit.txt
    fi
done
